...
class Cltvo_Repeater_With_Title extends Cltvo_Metabox_Master
{
	...
	public static function setMetaValue($meta_value){
		if (is_array($meta_value)) {
			$meta_value['title'] = (isset($meta_value['title']) && !empty($meta_value['title'])) ? $meta_value['title'] : '';
			$meta_value['items'] = (isset($meta_value['items']) && !empty($meta_value['items'])) ? $meta_value['items'] : array(0 => array('detalle' => ''));
			foreach ($meta_value['items'] as $key => $value) {
				foreach ((new static)->detalles as $detalle_key => $item) {
					$meta_value['items'][$key][$detalle_key] = (isset($meta_value['items'][$key][$detalle_key]) && !empty($meta_value['items'][$key][$detalle_key])) ? $meta_value['items'][$key][$detalle_key] : '';
				}
			}
		} else {
			$meta_value = array(
	...
	*/
	 public function CltvoDisplayMetabox($object){ ?>
		...
		<input type="text" style="width: calc(100% - 10px);" name="<?php echo $this->meta_key; ?>[title]" value="<?php echo $this->meta_value['title']; ?>" />
				...
				<tr class="clone-JS tr_sortable">
					<?php foreach ($this->detalles as $key => $item): ?>
						<td style="padding-bottom:10px;" input-name="<?php echo $key; ?>">
							...
							name="<?php echo $this->meta_key; ?>[items][][<?php echo $key; ?>]"
							value=""
							disabled />
						</td>
					<?php endforeach; ?>
	...
	public function drawTemplate($meta_value) {
		...
		foreach ($meta_value['items'] as $key_value => $value): ?>
			<tr class="tr_sortable" meta-key="<?php echo $key_value; ?>" id="<?php echo $this->meta_key; ?>_<?php echo $key_value; ?>">
				<?php foreach ($this->detalles as $key => $item): ?>
						<td style="padding-bottom: 10px;" input-name="<?php echo $key; ?>">
							...
							name="<?php echo $this->meta_key; ?>[items][<?php echo $key_value; ?>][<?php echo $key; ?>]"
